...
export function convertThinkingParameters(
	...
	switch (provider) {
		...
		case "openai":
			// For OpenAI, only disable streaming if tools are explicitly disabled
			// Do NOT disable streaming based on model type alone
			// Special handling for Deepseek models
			const isDeepseekModel = config?.model?.toLowerCase().includes("deepseek");

			// Log special handling for Deepseek models
			if (isDeepseekModel) {
				const logger = console.log || (() => {});
				logger(`🔍 Applying special handling for Deepseek model: ${config?.model}`);
			}

			return {
				...baseOptions,
				// Only disable streaming when tools are explicitly disabled
				stream: config?.toolsEnabled === false ? false : true,
				...(anthropicRequest.thinking?.type == "enabled" && {
					reasoningEffort: "medium" as const,
				}),
				// Ensure system message is properly formatted for Deepseek models
				...(isDeepseekModel &&
